...
async def create_city(db: AsyncSession, city: schemas.CityIn) -> schemas.City:
    ...
    new_city = schemas.City(id=new_city_id, name=city.name, additional_info=city.additional_info)
...
async def bulk_add_cities(db: AsyncSession):
    ...
    for city in cities_to_add:
        city_instance = models.City(name=city["name"], additional_info=city["additional_info"])
